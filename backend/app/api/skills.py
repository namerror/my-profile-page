...
def create_skill(skill_in: schemas.SkillCreate, db: Session = Depends(get_db), admin: str = Depends(get_current_admin)):
    return crud.create_skill_if_missing(db, name=skill_in.name, parent_id=skill_in.parent_id, category_id=skill_in.category_id)
...
def update_skill(skill_id: int, skill_in: schemas.SkillCreate, db: Session = Depends(get_db), admin: str = Depends(get_current_admin)):
    ...
    if not s:
        raise HTTPException(status_code=404, detail="Skill not found")
    return crud.update_skill(db, s, name=skill_in.name, parent_id=skill_in.parent_id, category_id=skill_in.category_id)
