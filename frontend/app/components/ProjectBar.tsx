...
export default function ProjectBar({ project }: ProjectBarProps) {
    return (
        <Link href={`/projects/${project.id}`}>
            <div className={`relative overflow-hidden rounded-2xl ring-1 ${project.is_completed ? "ring-gray-800/50" : "ring-gray-300/50"} shadow-lg hover:shadow-xl transition p-5 flex flex-col justify-between cursor-pointer h-full min-h-[150px] group mb-4`}>
                <Image
                ...
                <div className={`absolute inset-0 ${project.is_completed ? "bg-black/75 group-hover:bg-black/80" : "bg-white/60"} transition-colors`} />
                    ...
                    <div className="mt-auto space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                            {project.skills.map((skill, index) => (
                                <span
                                    key={index}
                                    className={`text-xs ${project.is_completed ? "bg-[#393939] text-[#fafafa]" : "bg-gray-200 text-[#212121]"} px-2 py-1 rounded-full`}
                                >
                                    {skill.name}
                                </span>
                            ))}
